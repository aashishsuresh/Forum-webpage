...
<center>
<?php
$usercontacts=$user_check."_contacts";
...
if($result)
{
	while($row = mysqli_fetch_array($result,MYSQLI_ASSOC))
	{
		...
		if($usertype=='alumni')
		{
		$sql2 = "SELECT * from alumni_profile where registerno='$userid'";
		...
		echo 'Studied From:'.$row2['batch'].'</br>';
		echo '</section>';
		}
		else
		 {
			 ...
			 echo 'Lives at:'.$row2['livesin'].'</br>';
			 echo 'Native Place'.$row2['native'].'</br>';
			 echo 'Currently Works at:'.$row2['worksat'].'</br>';
